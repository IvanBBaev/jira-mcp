...
> Status: target-state spec (pre-code). Every document describes the intended
> system; code may lag until the phase noted in IMPLEMENTATION-PLAN.md. Each
> document carries a status banner like this one.
...
| [CORNER-CASES.md](CORNER-CASES.md) | CC-01…CC-35 — stable ids referenced by tests and the plan. |
| [DECISIONS.md](DECISIONS.md) | Decision ledger: accepted D-rows, open owner O-rows, gates A–C. |
| [IMPLEMENTATION-PLAN.md](IMPLEMENTATION-PLAN.md) | Phases 0–5 with exits and milestones. |
